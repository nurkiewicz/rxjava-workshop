...
public class UrlDownloader {
	...
	public static Observable<String> downloadAsync(URL url, Scheduler scheduler) {
		return Observable
				.fromCallable(() -> downloadBlocking(url))
				.subscribeOn(scheduler);
	}
	
	public static Observable<String> download(URL url) {
		return Observable.fromCallable(() -> downloadBlocking(url));
	}
	
	/**
	 * Fails if more than `TOTAL` number of concurrent connections
	 */
	public static Observable<String> downloadThrottled(URL url) {
